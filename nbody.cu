...
int main(int argc, char **argv)
{
    ...
    if (error_id != cudaSuccess) {
        printf("CRITICAL ERROR: cudaGetDeviceCount returned %d\n-> %s\n", 
    ...
    if (deviceCount == 0) {
        printf("CRITICAL ERROR: There are no available device(s) that support CUDA\n");
        exit(1);
    } else {
        printf("SUCCESS: Found %d CUDA capable device(s).\n", deviceCount);
    }*/
    ...
    double finalEarthX = hPos[3][0];
    double difference = finalEarthX - initialEarthX;
    
    printf("DEBUG: Earth Final X:   %f\n", finalEarthX);
    printf("DEBUG: Total Change:    %f meters\n", difference);
    printf("DEBUG: Loop Iterations: %d\n", iterations);
	clock_t t1=clock()-t0;
#ifdef DEBUG
	printSystem(stdout);
#endif
